...
export default function GenresSection() {
  return (
    <section className="bg-slate-950 px-4 py-10 sm:py-16">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {genres.map(({ name, icon: Icon, color, iconColor, glow }) => (
            <Link
              key={name}
              href={`/books`}
              className={`group flex flex-col items-center gap-3 rounded-2xl border bg-gradient-to-b ${color} p-5 shadow-lg ${glow} transition-all duration-300 hover:-translate-y-1 hover:shadow-xl`}
            >
              <span
                className={`flex h-12 w-12 items-center justify-center rounded-xl bg-slate-900/60 text-2xl ${iconColor} transition-transform duration-300 group-hover:scale-110`}
              >
                <Icon />
              </span>
