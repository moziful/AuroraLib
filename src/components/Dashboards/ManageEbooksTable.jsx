...
export default function ManageEbooksTable({
  ...
  const filteredBooks = (books || []).filter((book) => {
    ...
    const writerMatch = book.writerName?.toLowerCase().includes(query);
    const genreMatch = book.genre?.toLowerCase().includes(query);
    
    const status = book.status?.toLowerCase() || "";
    let statusMatch = status.includes(query);
    if (query === "unpublish" || query === "unpublished") {
      statusMatch = statusMatch || status === "unavailable";
    } else if (query === "publish" || query === "published") {
      statusMatch = statusMatch || status === "available" || status === "published";
    }

    const priceMatch = String(book.price || "").toLowerCase().includes(query);
    
    return titleMatch || writerMatch || genreMatch || statusMatch || priceMatch;
  });
    ...
    <div className="space-y-4">
      ...
      <div className="lg:overflow-y-auto lg:overflow-x-hidden lg:max-h-[calc(100vh-280px)] pb-10 scrollbar-hide [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] scrollbar-none">
        <DataTable
          ...
          data={filteredBooks}
          isLoading={isLoading}
          emptyMessage={searchQuery ? "No matching ebooks found." : emptyMessage}
