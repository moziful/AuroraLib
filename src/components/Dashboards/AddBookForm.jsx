...
import { addBook, updateBook, revalidateBooks } from "@/lib/actions";

async function fetchAuthToken() {
  const res = await fetch("/api/auth/token");
  if (!res.ok) throw new Error("Failed to retrieve auth token.");
  ...
  useEffect(() => {
    if (initialData) {
      setForm({
        ...
        writerName: initialData.writerName || "",
        writerEmail: initialData.writerEmail || "",
        status: initialData.status || "Available",
        isFeatured: initialData.isFeatured || false,
      });
      if (initialData.coverImage || initialData.cover) {
        setUploadedCoverUrl(initialData.coverImage || initialData.cover);
        setCoverPreview(initialData.coverImage || initialData.cover);
      }
    } else if (user) {
      setForm((prev) => ({
        ...prev,
