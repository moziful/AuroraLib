    ...
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
      {books.map((book) => (
        ...
        >
          ...
          {book.writer && (
            <p className="text-xs text-slate-500 mt-0.5">{book.writer}</p>
          )}
          <Link
            href={`/books/${book.slug}`}
            className={`mt-3 block text-center rounded-lg bg-slate-800 py-1.5 text-xs font-semibold text-slate-300 transition-colors ${btnHoverClass} hover:text-slate-950`}
          >
            {actionLabel}
          </Link>
        </div>
